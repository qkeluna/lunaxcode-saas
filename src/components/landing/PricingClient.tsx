...
import { Check, ArrowRight, Clock, Sparkles, Shield, Zap, Building2, Rocket, Crown } from 'lucide-react';
...
// Plan differentiators for visual distinction
const planMeta: Record<string, { icon: React.ElementType; badge: string; color: string; gradient: string }> = {
  'Landing Page': {
    icon: Rocket,
...
// Default meta for plans not in the list
const defaultMeta = {
  icon: Sparkles,
  badge: '',
  color: 'violet',
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-16">
            {plans.map((plan, index) => {
              const meta = planMeta[plan.name] || defaultMeta;
              ...
              return (
                ...
                >
                  {/* Top badge - differentiator */}
                  {meta.badge && (
                    <div className={`absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-gradient-to-r ${meta.gradient} text-white text-sm font-medium rounded-full shadow-lg flex items-center gap-1.5`}>
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    {/* Plan icon */}
                    <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${meta.gradient} flex items-center justify-center mb-6 transition-transform duration-300 ${isHovered ? 'scale-110' : ''}`}>
                    ...
                    {/* Timeline with icon */}
                    <div className={`flex items-center gap-2 mb-6 px-3 py-2.5 rounded-xl text-sm ${
                      plan.popular
                        ? 'bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800'
                        : 'bg-muted'
                    }`}>
                      <Clock className={`w-4 h-4 ${plan.popular ? 'text-violet-600 dark:text-violet-400' : 'text-muted-foreground'}`} aria-hidden="true" />
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-3 gap-8 mt-16 pt-16 border-t border-border transition-all duration-700 delay-800 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="text-center p-6 rounded-2xl bg-card border border-border hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors">
            ...
            <div className="text-center p-6 rounded-2xl bg-card border border-border hover:border-violet-200 dark:hover:border-violet-800 transition-colors">
            ...
            <div className="text-center p-6 rounded-2xl bg-card border border-border hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
